  ...
  try {
    ...
    if (action === 'aucDraftSuggest') { // 🤔 모르겠음 — AI가 이번 차례 픽/밴 추천 (솔랭·자랭 추정 + 내전 데이터 · BYOK 지원)
      ...
      var sgPrompt = '리그 오브 레전드 5대5 토너먼트 드래프트 진행 중. 나는 ' + sgSide + '팀이고 지금 우리 팀 ' + sgType + ' 차례입니다. 프로 드래프트 코치처럼, 아래 정보를 "모두" 종합해 이번 차례 최적의 한 수를 결정하세요.\n\n'
        ...
        + (body.type === 'ban'
            ? '【밴 추천 — 솔랭+내전 데이터 기반, 상대가 "그 라인에서" 실제 쓰는 챔프만】\n(1) 상대 각 선수의 포지션을 보고, 그 선수가 그 라인에서 실제로 쓰는 주력 챔프(위 상대 챔프풀 N겜W%)만 밴 후보. 그 선수 모스트라도 그 라인 챔프가 아니면(예: 탑 선수인데 정글 챔프 브라이어) 제외 — 그 라인으로 안 가져오니 밴 의미 없음.\n(2) 후보 중 가장 위협적인 1개(숙련도 높고 라인전/한타 영향 크고 우리 빈 라인에 까다로운 것)를 끊는다.\n(3) 진영: 블루면 상대 선픽 라인OP 선제 차단, 레드면 상대 막픽 카운터 카드 제거.'
        ...
        + '위 기준을 머릿속으로 따져 최선 1개만 고르세요. 분석을 길게 쓰지 말고 결과만 짧게.\n'
        + '출력 형식(딱 2줄):\n'
        + '1줄째: "추천' + sgType + ': " 뒤에 추천 챔피언 이름 하나만(아래 목록 표기 그대로). 예) 추천' + sgType + ': 브라이어\n'
        + '2줄째: 한 줄 이유(누구 라인·왜를 한 문장으로 짧게. 예: "탑 사과 모스트1 17겜68%, 라인전 카운터 어려움").\n\n'
        + '선택 가능 챔피언: ' + avail.join(', ');
      try {
        var sgText = await callLLM(sgPrompt, 400, body.userKey);
